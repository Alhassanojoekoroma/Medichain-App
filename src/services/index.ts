...
const GATEWAY_URL = process.env.EXPO_PUBLIC_HYPERLEDGER_GATEWAY_URL || 'http://localhost:3001';
    ...
    if (GATEWAY_URL) {
      try {
        const response = await fetch(`${GATEWAY_URL}/access/grant`, {
          ...
          body: JSON.stringify({ doctorId, expiryMinutes })
        });
        const data = await response.json();
        return data.txHash;
      } catch (e) {
        console.warn('Blockchain Gateway unreachable, falling back to local.', e);
      }
    }
    ...
    if (GATEWAY_URL) {
      try {
        const response = await fetch(`${GATEWAY_URL}/records/notarize`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ recordId, contentHash })
        });
        const data = await response.json();
        return data.txHash;
      } catch (e) {
