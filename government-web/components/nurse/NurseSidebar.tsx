...
const navItems = [
  ...
  { href: '/nurse/patients', icon: Users, label: 'Patients' },
  { href: '/nurse/appointments', icon: Calendar, label: 'Appointments' },
  { href: '/nurse/scan', icon: QrCode, label: 'Scan QR' },
  { href: '/nurse/records', icon: FileText, label: 'Records' },
  { href: '/nurse/notifications', icon: Bell, label: 'Notifications' },
];

const helpNavItems = [
  { href: '/nurse/settings', icon: Settings, label: 'Settings' }, // redirect to settings
];

export default function NurseSidebar({ isOpen, onToggle }: SidebarProps) {
      ...
      >
        ...
        <div className="px-4 flex-1 pt-4 overflow-y-auto">
          <nav className="space-y-1">
            {navItems.map((item) => {
              const isActive = pathname === item.href || (item.href !== '/nurse/dashboard' && pathname.startsWith(item.href));
              return (
                <a
                  key={item.href}
                  href={item.href}
